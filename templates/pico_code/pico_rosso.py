...
def loop():
    ...
    while True:
        ...
        if current_weapon == "spada":
            ...
            pin_14 = Pin(PIN_B_NUM, Pin.OUT); pin_14.value(0) # Il mio giubbetto a massa fissa per l'avversario
            pin_15 = Pin(PIN_C_NUM, Pin.IN, Pin.PULL_UP)      # Ritorno del pulsante NC
            
            # --- FASE 1: Rilevo se si apre nel vuoto ---
            pin_13.value(0)
            time.sleep_us(500)
            read_phase1 = pin_15.value()
            
            # --- FASE 2: Rilevo se tocca il giubbetto avversario ---
            pin_13.value(1)
            time.sleep_us(500)

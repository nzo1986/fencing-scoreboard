...
# ==========================================
# MOTORE 1: SPADA (MULTIPLEXING 2 FASI)
# ==========================================
def run_spada():
    PIN_13 = Pin(13, Pin.OUT); PIN_13.value(0) # Coccia fissa a Massa (0V)
    ...
    while current_weapon == "spada":
        ...
        # --- FASE 1: Lettura Massa (Coccia) ---
        PIN_14.init(Pin.IN, Pin.PULL_UP)
        PIN_15.init(Pin.IN, Pin.PULL_UP)
        time.sleep_us(200)
        touch_gnd = (PIN_14.value() == 0 or PIN_15.value() == 0)

        # --- FASE 2: Lettura Punta ---
        is_hit = False
        if not touch_gnd:
            PIN_14.init(Pin.OUT); PIN_14.value(0) # Manda corrente dal Polo 1
            time.sleep_us(200)
            is_hit = (PIN_15.value() == 0)        # Legge dal Polo 2
            PIN_14.init(Pin.IN, Pin.PULL_UP)      # Ripristino
